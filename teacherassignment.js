...
let a = document.querySelector("#assignment button");
...
function closeAllForms() {
    ...
    let editBtn = document.querySelector("#assignment #edit");
    if (editBtn) {
        editBtn.remove();
        ...
        saveBtn.id = "save";
        saveBtn.type = "submit";
        document.querySelector("#assignment").appendChild(saveBtn);
    }
}
...
dc.get().then((s) => {
    ...
    s.forEach((snap) => {
        ...
        btn2.addEventListener("click", () => {
            ...
            let existingEditBtn = document.querySelector("#assignment #edit");
            if (existingEditBtn) {
                existingEditBtn.remove();
            }
            // Remove existing save button if present
            let existingSaveBtn = document.querySelector("#assignment #save");
            if (existingSaveBtn) {
            ...
            btnedit.id = "edit";
            document.querySelector("#assignment").appendChild(btnedit);
            // styling
...
let api1 = "https://openrouter.ai/api/v1/chat/completions";
...
async function chat() {
    ...
    try {
        let db1 = await firebase.firestore();
        let currentPage = window.location.pathname;
        let dataContext = {};

        if (currentPage.includes('assignment')) {
            let da = await db1.collection("assignment");
            let dsa = await db1.collection("student_assignment");
            
            let [data2, data3] = await Promise.all([da.get(), dsa.get()]);
            
            let alldata_assignments = [];
            let alldata_student_assignments = [];
            
            data2.forEach((e) => alldata_assignments.push(e.data()));
            data3.forEach((e) => alldata_student_assignments.push(e.data()));
            
            dataContext = {
                type: "teacher",
                assignments: alldata_assignments,
                ...
                messages: [
                    {
                        role: "user",
                        content: `
You are a helpful and friendly ${dataContext.type === 'teacher' ? 'teacher' : 'student'} assistant.
...
DATA:
this is assignments=> ${JSON.stringify(dataContext.assignments)}
this is student who submitted the assignment=> ${JSON.stringify(dataContext.student_assignments)}
