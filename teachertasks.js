...
let a = document.querySelector("#tasks button");
...
function closeAllForms() {
    ...
    let editBtn = document.querySelector("#tasks #edit");
    if (editBtn) {
        editBtn.remove();
        ...
        saveBtn.id = "save";
        saveBtn.type = "submit";
        document.querySelector("#tasks").appendChild(saveBtn);
    }
}
...
dc.get().then((s) => {
    ...
    s.forEach((snap) => {
        ...
        btn2.addEventListener("click", () => {
            ...
            let existingEditBtn = document.querySelector("#tasks #edit");
            if (existingEditBtn) {
                existingEditBtn.remove();
            }
            // Remove existing save button if present
            let existingSaveBtn = document.querySelector("#tasks #save");
            if (existingSaveBtn) {
            ...
            btnedit.id = "edit";
            document.querySelector("#tasks").appendChild(btnedit);
            // styling
...
async function chat() {
    ...
    try {
        ...
        if (currentPage.includes('teacher')) {
            let dq = await db1.collection("tasks");
            let data3 = await dq.get();
            let students = await db1.collection("student_tasks");
            let alldata_tasks = [];
            data3.forEach((e) => {
                alldata_tasks.push(e.data());
            });
            let data4 = await students.get();
            data4.forEach((e)=> {
                student_tasks.push(e.data());
            })
            dataContext = {
                type: "teacher",
                tasks: alldata_tasks,
...
DATA:
all tasks =>  ${JSON.stringify(dataContext)}
